...
print("hide mouse	   :", opt.nomouse)
print("hide frequency  :", opt.showfreq)
print("hide infos	   :", opt.info_phase)
print("fontsize		:", opt.fontsize)
print("freq control		 :", opt.control)
print("freq ticks interval  :", opt.freqgridticksinterval)
print("label freq ticks interval  :", opt.freqlabelticksinterval)
...
if opt.screenresolution:
	SCREENW=int(opt.screenresolution.split("x")[0])
	SCREENH=int(opt.screenresolution.split("x")[1])
...
class Graticule(object):
	...
	def make(self):
		...
		for xtick_max in [800, 400, 200, 100, 80, 40, 20, 10]:
			if xtick_max < srate2:
				break
		if opt.freqgridticksinterval:
			maxfreq=int(str(frq_range)[:1])*(pow(10,len(str(int(frq_range)))-1))
			print maxfreq
			ticks = range(-int(maxfreq)/2, int(maxfreq)/2 + opt.freqgridticksinterval, opt.freqgridticksinterval)
			print ticks
		else:
...
if opt.lcd4:  # setup for directfb (non-X) graphics
	...
	SCREEN_SIZE = (SCREENW, SCREENH) if opt.waterfall \
...
h_2d =  (SCREEN_SIZE[1] * opt.scalscreensep)/100 if opt.waterfall \
	else SCREEN_SIZE[1]  # height of 2d spectrum display
if not opt.hideborders:
...
# Waterfall geometry
h_wf = (SCREEN_SIZE[1] - h_2d -y_2d - 5)  # Height of waterfall (3d spectrum)
if opt.hideborders:
	h_wf = (SCREEN_SIZE[1] - h_2d -y_2d)
...
smfont = pg.font.SysFont('mono', opt.fontsize, True)
...
info_phase = not opt.info_phase  # > 0 --> show info overlay
info_counter = 0
tloop = 0.
t_last_data = 0.
nframe = 0
